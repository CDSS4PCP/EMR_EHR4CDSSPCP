...
const patientId = "b088b066-58cf-49c4-a9e0-f48564c531c0";

async function getPatient() {

  const response = await fetch("http://localhost:8080/openmrs/ws/fhir2/R4/Patient/" + patientId);
  const patient = await response.json();
  console.log(patient);

  let result = await global.cdss.executeCql(patient, rule, { "FHIRHelpers": fhirHelpers }, {});
  console.log(result);
  return result;
}
...
const Root: React.FC = () => {
  ...
  let [patientResults, setPatientResults] = useState({});


  function toggle() {
    getPatient().then(result => setPatientResults(result));
  ...
  return (
    <div>
      ...
      <div className={styles.container}>
        ...
        <Button className={'cds--btn cds--layout--size-lg'} title={'Do cool stuff!'} onClick={e => toggle()}>Do CDSS</Button>
        {Object.keys(patientResults).length > 0 && <Table>
          <TableHead>
          <TableRow>
            <TableCell>Patient</TableCell>
            <TableCell>InPopulation</TableCell>
          </TableRow>
          </TableHead>
          <TableBody>
          <TableRow>
            <TableCell>{patientId}</TableCell>
            <TableCell>{patientResults[patientId].InPopulation + ""}</TableCell>
          </TableRow>
          </TableBody>
        </Table>}
      </div>
