...
/**
 * A projection that extracts only the `id` from a list of RuleDescriptors.
 *
 * This projection transforms a list of RuleDescriptors into a list of strings,
 * each representing the `id` of a RuleDescriptor.
 */
public class IdProjection extends RuleProjection<String> {
    ...
    public List<String> apply(List<RuleDescriptor> rules) {
        return rules.stream()
                .map(RuleDescriptor::getId)  // Extract the `id` from each RuleDescriptor
